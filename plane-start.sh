...
update_env_file(){
    ...
    # Build port suffix from APP_PORT env var (omit if 80/443)
    local port_suffix=""
    [ -n "$APP_PORT" ] && [ "$APP_PORT" != "80" ] && [ "$APP_PORT" != "443" ] && port_suffix=":$APP_PORT"

    update_env_value "APP_DOMAIN" "$DOMAIN_NAME$port_suffix"
    if [ -n "$SITE_ADDRESS" ]; then
        update_env_value "SITE_ADDRESS" "$SITE_ADDRESS"
    else
        update_env_value "SITE_ADDRESS" ":80"
    fi
    update_env_value "WEB_URL" "$app_protocol://$DOMAIN_NAME$port_suffix"
    update_env_value "CORS_ALLOWED_ORIGINS" "http://$DOMAIN_NAME$port_suffix,https://$DOMAIN_NAME$port_suffix"
